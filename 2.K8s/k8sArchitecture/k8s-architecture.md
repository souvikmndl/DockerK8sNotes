...
```mermaid
flowchart TB
    Client([kubectl / Client]) -->|Update / Query| API

    subgraph CP["Control Plane (master)"]
        API["kube-apiserver<br/>front door: authn / authz / admission"]
        SCHED["kube-scheduler<br/>picks a node"]
        CM["controller-manager<br/>reconciliation loops"]
        ETCD[("etcd<br/>cluster brain / KV store")]
        API <-->|only writer| ETCD
        SCHED -->|watch / bind| API
        CM -->|watch / act| API
    end

    subgraph N1["Worker Node 1  (30% used)"]
        K1["kubelet"]
        P1["kube-proxy"]
        R1["containerd / CRI-O"]
        POD1["Pod<br/>containers"]
        K1 --> R1 --> POD1
    end

    subgraph N2["Worker Node 2  (60% used)"]
        K2["kubelet"]
        P2["kube-proxy"]
        R2["containerd / CRI-O"]
        POD2["Pod<br/>containers"]
        K2 --> R2 --> POD2
    end

    API <-->|"watch assigned pods / report status"| K1
    API <-->|"watch assigned pods / report status"| K2
```
...
```mermaid
flowchart LR
    A["Desired state<br/>(what you declared in etcd)"] --> C{Compare}
    B["Current state<br/>(what actually exists)"] --> C
    C -->|drift detected| ACT["Act: create / delete / update<br/>to close the gap"]
    ACT --> B
    C -->|match| WAIT["Do nothing, keep watching"]
    WAIT --> B
```
...
sequenceDiagram
    autonumber
    actor U as You (kubectl)
    participant API as kube-apiserver
    participant E as etcd
    participant CM as controller-manager
    participant S as kube-scheduler
    participant KL as kubelet (chosen node)
    participant CR as container runtime

    U->>API: apply deployment.yaml
    API->>API: authn / authz / admission / validate
    API->>E: persist Deployment (desired state)
    CM->>API: watch -> new Deployment
    CM->>API: create ReplicaSet + Pods (no node yet)
    API->>E: persist Pods (unscheduled)
    S->>API: watch -> unscheduled pods
    S->>S: filter feasible nodes, then score
    S->>API: bind pod to Node
    API->>E: persist node assignment
    KL->>API: watch -> pod assigned to me
    KL->>CR: pull image, start container(s)
    CR-->>KL: running
    KL->>API: report pod status (Running)
    API->>E: state now matches reality
```
